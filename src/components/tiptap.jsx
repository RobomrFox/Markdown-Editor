...
const MenuBar = () => {
  ...
  <div className="group relative bg-white/90 backdrop-blur-md border border-slate-200 p-2 my-4 rounded-lg shadow-sm flex gap-4 items-center max-w-[90%] mx-auto transition-all duration-300 hover:shadow-md">
    ...
    <button
      ...
      className={`p-2 rounded-md transition-colors ${editor.isActive('bold') ? 'bg-slate-100 text-blue-600' : 'hover:bg-slate-50'}`}
    >
      <img src="src/assets/1_MenuBar/1_bold.svg" alt='Bold' className='h-6' />
    </button>

    <button
      onClick={() => editor.chain().focus().toggleItalic().run()}
      disabled={!editor.can().chain().focus().toggleItalic().run()}
      className={`p-2 rounded-md transition-colors ${editor.isActive('italic') ? 'bg-slate-100 text-blue-600' : 'hover:bg-slate-50'}`}
    >
      <img src="src/assets/1_MenuBar/2_italic.svg" alt='Italics' className='h-6' />
    </button>

    <button
      onClick={() => editor.chain().focus().toggleStrike().run()}
      disabled={!editor.can().chain().focus().toggleStrike().run()}
      className={`p-2 rounded-md transition-colors ${editor.isActive('strike') ? 'bg-slate-100 text-blue-600' : 'hover:bg-slate-50'}`}
    >
      <img src="src/assets/1_MenuBar/3_Strike.svg" alt='Strike' className='h-6' />
    </button>
    ...
    <button
      ...
      className={`p-2 rounded-md transition-colors ${editor.isActive('heading', { level: 1 }) ? 'bg-slate-100 text-blue-600' : 'hover:bg-slate-50'}`}
    >
    ...
    <button
      ...
      className={`p-2 rounded-md transition-colors ${editor.isActive('heading', { level: 2 }) ? 'bg-slate-100 text-blue-600' : 'hover:bg-slate-50'}`}
    >
      <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="size-6">
    ...
    <button
      ...
      className={`p-2 rounded-md transition-colors ${editor.isActive('heading', { level: 3 }) ? 'bg-slate-100 text-blue-600' : 'hover:bg-slate-50'}`}
    >
      <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="size-6">
    ...
    <button
      ...
      className={`p-2 rounded-md transition-colors ${editor.isActive('bulletList') ? 'bg-slate-100 text-blue-600' : 'hover:bg-slate-50'}`}
    >
    ...
    <button
      ...
      className={`p-2 rounded-md transition-colors ${editor.isActive('orderedList') ? 'bg-slate-100 text-blue-600' : 'hover:bg-slate-50'}`}
    >
    ...
    <button
      ...
      className={`p-2 rounded-md transition-colors ${editor.isActive('codeBlock') ? 'bg-slate-100 text-blue-600' : 'hover:bg-slate-50'}`}
    >
      <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="size-6">
